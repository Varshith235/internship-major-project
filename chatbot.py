...
class SupportCopilot:
    ...
    def _handle_math(self, query):
        ...
        # Clean query: remove 'what is', 'calculate', etc.
    ...
    def _handle_science_and_knowledge(self, query):
        ...
        # Simple regex to check if it's a general question
        match = re.match(r'(what is|who is|tell me about)\s+(.*)', query, flags=re.IGNORECASE)
        if match:
    ...
    def _handle_places(self, query):
        ...
        if re.search(r'(nearby|places to visit|good places near)', query, flags=re.IGNORECASE):
            ...
            if match:
                ...
                try:
                    # Let's try to fetch something about the city
    ...
    def process_query(self, user_query):
        ...
        intent, confidence = self.classifier.predict_intent(user_query)
        
        # Smart Routing: Check for escalation
        ...
        rag_answer = self.retriever.get_relevant_answer(user_query)
        ...
        if rag_answer:
            ...
            return (
                ...
                "I couldn't find an exact match in our FAQs, but I can help you file a ticket or "

...
if not st.session_state.logged_in:
    ...
    if module_selection == "User Module":
        ...
        st.info("Predefined Admins: admin1 to admin5 (password: admin123)")
    ...
    if st.session_state.role == 'admin':
        ...
        tab_pending, tab_resolved, tab_users, tab_all_queries, tab_train_ai = st.tabs([
            "Pending Tickets", "Resolved Tickets", "User Database", "All User Queries", "🧠 Train AI"
        ])
        ...
        with tab_train_ai:
            st.header("Train AI (Dynamic Knowledge Base)")
            st.markdown("Add new information to the AI's brain. It will instantly learn this and answer future questions!")
            
            with st.form("train_ai_form"):
                new_q = st.text_input("User Question (e.g. 'Do you ship internationally?')")
                new_a = st.text_area("Ideal AI Answer (e.g. 'Yes, we ship to over 100 countries.')")
                submit_train = st.form_submit_button("Train AI")
                
                if submit_train:
                    if new_q and new_a:
                        success = database.add_knowledge(new_q, new_a)
                        if success:
                            # Retrain the AI instantly with the new data
                            bot.retriever.retrain()
                            st.success("✨ AI successfully trained on new knowledge! It will now answer this instantly.")
                        else:
                            st.error("This exact question already exists in the knowledge base.")
                    else:
                        st.error("Please provide both a question and an answer.")
                        
            st.subheader("Current Knowledge Base")
            kb_df = database.get_knowledge_base()
    ...
    # --- USER MODULE ---
        ...
        with tab1:
            ...
            cols = st.columns(4)
            suggestions = ["I need to speak to a manager", "What is 15 * 8?", "Tell me a joke", "Places to visit in Paris"]
